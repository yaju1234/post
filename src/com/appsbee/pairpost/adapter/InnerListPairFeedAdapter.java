...
public class InnerListPairFeedAdapter  extends ArrayAdapter<Discoder> {
    ...
    private InnerTabPairPost tabPairFeedFragment;
    private  Dialog dlg_comments;
    private  Dialog dlg_list;

    public InnerListPairFeedAdapter(InnerTabPairPost tabPairFeedFragment,Activity activity, int textViewResourceId) {
    ...
    public void commentDlg( final String post_id,final String totalcomment) {
  	...
  	InnerTabCommentAdapter listCommentAdapter = new InnerTabCommentAdapter(InnerListPairFeedAdapter.this,Constant.userId,post_id,activity,R.layout.row_comments,mCommentArr);
	...
	protected Boolean doInBackground(String... params) {
		...
		if(response!=null){
			 ...
			 for (int i=0; i< jArr.length(); i++){
			     ...
			     mCommentArr.add(new Comments(id,image, name,"", comment, time));
			 }
		     }
		}
	    } catch (Exception e) {
	...
	String totalcomment;
  	...
  	protected void onPostExecute(Boolean result) {
  	   ...
  	   if(result){
  	     ...
  	     mCommentArr.add(new Comments(Constant.userId,Constant.userImage, Constant.userName, "", comments, ""));
